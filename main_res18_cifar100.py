...
parser.add_argument('--result_dir', type=str, default='checkpoint_cifar100_res18')
...
def main():
    ...
    for epoch in range(args.start_epoch, args.end_epoch):
        ...
        if not os.path.exists('./AVmixup_res18_cifar100'):
            os.mkdir('./AVmixup_res18_cifar100')
        torch.save(net.state_dict(), './AVmixup_res18_cifar100/ckpt.{}'.format(epoch))
...
def get_dataset(transform_trainlist, transform_testlist):
    ...
    if args.dataset == 'svhn':
        ...
        transform_trainlist.insert(1, transforms.RandomHorizontalFlip())
        trainset = use_dataset(root='../../data', train=True, download=True, transform=transforms.Compose(transform_trainlist))
        testset = use_dataset(root='../../data', train=False, download=True, transform=transforms.Compose(transform_testlist))
        depth, widen_factor = 34, 10
        model_type = 'Wideres28'
    print('==> Model type: %s' % model_type)

    if args.dataset == 'tiny_imagenet':    net=resnet18(pretrained=True, num_classes=args.n_cls)
    else:    
        net=ResNet18(num_classes=100, conv1_size=3,activation_fn=nn.ReLU).to(device)
        #net=WideResNet(depth=depth, num_classes=args.n_cls, widen_factor=widen_factor)
